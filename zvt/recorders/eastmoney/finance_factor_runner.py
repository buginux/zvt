...
@sched.scheduled_job('cron', day_of_week='sun,thu', hour=1, minute=00)
def run():
    err_count = 0
    ...
    while True:
        try:
            ...
            wework.send_finished_message('FinanceFactor')
            err_count = 0
            break
            ...
            if err_count >= 10:
                wework.send_message(f'FinanceFactor 下载出错超过 {err_count} 次, 请检查...\n{e}')
                err_count = 0

            logger.exception('finance factor runner error:{}'.format(e))
            time.sleep(60)
